...
// Tripwires: the deployed-app path only works if the Worker server entry
// wraps grok-pwa middleware — an accidental edit that drops that wiring
// would otherwise fail silently (published apps would just render the app
// for ?install=1).
test("vite config keeps grok PWA plugin and worker server entry", () => {
  const viteConfig = readFileSync(join(TEMPLATE_ROOT, "vite.config.ts"), "utf8");
  assert.match(viteConfig, /grokPwaPlugin\(\)/);
  const startEntry = readFileSync(join(TEMPLATE_ROOT, "src/start.ts"), "utf8");
  assert.match(startEntry, /grokPwaMiddleware/);
});
